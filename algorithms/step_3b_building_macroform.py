...
from qgis.core import (
    ...
    QgsField, QgsFields, QgsFeatureSink
)
...
from core.macroform_engine import load_templates, match_template_to_bbox, fit_template_to_bbox
...
class BuildingMacroformAlgorithm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        self.addParameter(QgsProcessingParameterFeatureSource(
            self.INPUT_BUILDINGS, self.tr('Bina taban alanı katmanı (Adım 3 çıktısı)'),
            [QgsProcessing.TypeVectorPolygon]))
        self.addParameter(QgsProcessingParameterFeatureSource(
            self.INPUT_PARCELS, self.tr('Parsel katmanı'),
            [QgsProcessing.TypeVectorPolygon]))
        self.addParameter(QgsProcessingParameterFile(
            self.TEMPLATE_FILE, self.tr('Şablon bina formları (.gpkg)'),
            extension='gpkg'))
        self.addParameter(QgsProcessingParameterEnum(
            self.DIVERSITY, self.tr('Form çeşitlilik seviyesi'),
            options=['Düşük', 'Orta', 'Yüksek'], defaultValue=1))
        self.addParameter(QgsProcessingParameterNumber(
            self.MAX_UTILIZATION, self.tr('Maks alan kullanımı (%)'),
            QgsProcessingParameterNumber.Double, 95.0,
            minValue=50.0, maxValue=100.0))
        self.addParameter(QgsProcessingParameterBoolean(
            self.ROTATE_TO_FIT, self.tr('Sığdırmak için döndür'),
            defaultValue=True))
        self.addParameter(QgsProcessingParameterNumber(
            self.RANDOM_SEED, self.tr('Rastgele tohum (0=rastgele)'),
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        max_util = self.parameterAsDouble(parameters, self.MAX_UTILIZATION, context) / 100.0
        ...
        out_fields.append(QgsField('macroform_alan_m2', QVariant.Double, 'double', 20, 2))
        ...
        for i, feat in enumerate(source.getFeatures()):
            ...
            tmpl = match_template_to_bbox(bbox_geom, templates, diversity, target_area, rng)
            if tmpl is None:
                # Fallback: orijinal bbox geometrisini kullan
                nf = QgsFeature(out_fields)
                nf.setGeometry(bbox_geom)
                attrs = list(feat.attributes()) + ['orijinal', round(bbox_geom.area(), 2)]
            ...
            if fitted is None or fitted.isEmpty():
                nf = QgsFeature(out_fields)
                nf.setGeometry(bbox_geom)
                attrs = list(feat.attributes()) + ['fallback', round(bbox_geom.area(), 2)]
                nf.setAttributes(attrs)
                sink.addFeature(nf, QgsFeatureSink.FastInsert)
                continue
