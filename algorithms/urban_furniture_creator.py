...
FURNITURE_CATALOG = [
    ...
    ('aydinlatma',      'Aydınlatma Direği',          'aydinlatma',    'furniture_light.svg',    '#FFD700', 4.0),
    ('cop_kovasi',      'Çöp Kovası',                 'temizlik',      'furniture_trash.svg',    '#4A5568', 3.5),
    ('yangin_muslubu',  'Yangın Musluğu',             'guvenlik',      'furniture_hydrant.svg',  '#E53E3E', 4.0),
    ('agac_donati',     'Süs Ağacı / Ağaç (donatı)',  'peyzaj',        'furniture_tree.svg',     '#38A169', 6.0),
    ('bilgi_panosu',    'Bilgi Panosu / Tabela',      'bilgilendirme', 'furniture_infoboard.svg','#4A5568', 4.0),
    ('cesme',           'Çeşme / Su Öğesi',           'peyzaj',        'furniture_fountain.svg', '#3182CE', 5.0),
    ('bisiklet_park',   'Bisiklet Parkı',             'ulasim',        'furniture_bench.svg',    '#DD6B20', 4.5),
    ('elektrik_sarj',   'EV Şarj İstasyonu',          'ulasim',        'furniture_light.svg',    '#38B2AC', 4.5),
    ('oyun_alani',      'Çocuk Oyun Alanı İşareti',   'rekreasyon',    'furniture_fountain.svg', '#ED8936', 7.0),
    ('araba',           'Araba',                      'ulasim',        'furniture_car.svg',      '#607D8B', 15.0),
    ('trafik_isigi',    'Trafik Işığı',               'ulasim',        'furniture_traffic_light.svg', '#333333', 3.0),
]
...
class UrbanFurnitureAlgorithm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(QgsProcessingParameterFeatureSink(
            self.OUTPUT,
            self.tr('Kentsel donatı katmanı (hazır stillenmiş)')))
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        for i, (did, name, cat, svg, color, size) in enumerate(FURNITURE_CATALOG):
            ...
            f.setAttributes([
                did, name, cat, svg_path, color, size,
                f"Yerleşim planı donatısı: {name}. Bu noktayı kopyalayıp istediğiniz konuma taşıyın."
            ])
            sink.addFeature(f, QgsFeatureSink.FastInsert)

        feedback.pushInfo(f"✅ {len(FURNITURE_CATALOG)} adet kentsel donatı şablonu oluşturuldu.")
        feedback.pushInfo("")
        ...
        feedback.pushInfo("   (Sadece açılır listeden 'donati_id' seçmeniz yeterlidir!)")
        feedback.pushInfo("")
    ...
    def postProcessAlgorithm(self, context, feedback):
        ...
        for field_name in hidden_fields:
            ...
            if idx >= 0:
                output_layer.setEditorWidgetSetup(idx, QgsEditorWidgetSetup('Hidden', {}))
