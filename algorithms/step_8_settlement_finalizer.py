...
class SettlementFinalizerAlgorithm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(QgsProcessingParameterFeatureSource(
            ...
            [QgsProcessing.TypeVectorPolygon]))
        self.addParameter(QgsProcessingParameterFeatureSource(
            self.INPUT_TREES, self.tr('Ağaç katmanı (opsiyonel)'),
            [QgsProcessing.TypeVectorPoint], optional=True))
        self.addParameter(QgsProcessingParameterFeatureSource(
            self.INPUT_HARDSURFACE, self.tr('Sert zemin katmanı (opsiyonel)'),
            [QgsProcessing.TypeVectorPolygon], optional=True))
        self.addParameter(QgsProcessingParameterFeatureSource(
            self.INPUT_PARKING, self.tr('Otopark stall katmanı (opsiyonel)'),
            [QgsProcessing.TypeVectorPolygon], optional=True))
        self.addParameter(QgsProcessingParameterField(
            self.TAKS_FIELD, self.tr('TAKS sütunu'),
            parentLayerParameterName=self.INPUT_PARCELS, optional=True))
        self.addParameter(QgsProcessingParameterField(
            self.KAKS_FIELD, self.tr('KAKS/Emsal sütunu'),
            parentLayerParameterName=self.INPUT_PARCELS, optional=True))
        self.addParameter(QgsProcessingParameterNumber(
            self.FLAT_SIZE,
            self.tr('Ortalama daire büyüklüğü (m²) — konut istatistikleri için'),
            QgsProcessingParameterNumber.Double, 120.0, minValue=40.0))
        self.addParameter(QgsProcessingParameterNumber(
            self.HOUSEHOLD_SIZE,
            self.tr('Ortalama hane halkı büyüklüğü — 2026 TÜİK ortalaması ~2.77'),
            QgsProcessingParameterNumber.Double, 2.77, minValue=1.0))
        self.addParameter(QgsProcessingParameterFeatureSink(
            self.OUTPUT_STATS, self.tr('Genel istatistik raporu')))
        self.addParameter(QgsProcessingParameterFeatureSink(
            self.OUTPUT_PARKING_REPORT, self.tr('Otopark yeterliliği raporu (parsel bazlı)')))
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        taks_f = self.parameterAsString(parameters, self.TAKS_FIELD, context)
        ...
        if not p_src or not b_src:
            raise QgsProcessingException(self.tr("Parsel ve bina katmanları zorunludur."))
        ...
        (stat_sink, stat_dest) = self.parameterAsSink(
            parameters, self.OUTPUT_STATS, context,
            stat_fields, QgsWkbTypes.NoGeometry, p_src.sourceCrs())
        ...
        park_fields.append(QgsField('toplam_insaat_m2', QVariant.Double, 'double', 20, 2))
        ...
        (park_sink, park_dest) = self.parameterAsSink(
            parameters, self.OUTPUT_PARKING_REPORT, context,
            park_fields, QgsWkbTypes.MultiPolygon, p_src.sourceCrs())
        ...
        for pf in p_src.getFeatures():
            ...
            nf.setGeometry(pg)
            nf.setAttributes([
                pf.id(), round(p_area, 2), round(kaks_val, 4),
                round(tia, 2), tahmini_daire, round(tahmini_nufus, 1),
                gerekli_otopark, mevcut, yeterli, fark
            ])
        ...
        stats = [
            ...
            ('ortalama_taks', total_building_area / total_parcel_area if total_parcel_area > 0 else 0, 'oran'),
